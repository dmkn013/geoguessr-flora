...
#tip.show{opacity:1}
/* ホバーが無い環境ではツールチップ内のリンクを押せる必要がある。
   pointer-events:none のままだと地図リンクがタップできない。 */
@media (hover:none){
  /* 点の座標に合わせて置くと、端の点で画面からはみ出す（実機で確認）。
     タッチでは位置を追わず、地図の下側に幅いっぱいで固定する。 */
  #tip{
    pointer-events:auto;
    position:fixed; left:12px; right:12px; top:auto; bottom:12px;
    max-width:none; transform:none; z-index:20;
  }
  #tip .gmap{
    display:block; margin-top:.45rem; padding:.4rem .5rem; text-align:center;
...
<header>
  <h1>植生メタアトラス</h1>
  <p class="sub">見えた植物から地域を絞り込むための地図。<strong>白縁の点は、その地点の車載写真に実際に写っていることを目視で確認した92点</strong>。それ以外の点は分布域から機械的に散らしたもので、1点ごとの確認は取っていない（分布の濃さの表示）。色は種類ではなく、近くに生える種どうしを見分けるための10色。<strong>白縁の点をタップ（PCではホバー）すると、その地点の実写が出る</strong>。</p><p class="sub2">各種の詳細に<strong>遭遇率</strong>を載せた。分布域をランダムに40回引き、車載写真にその植物が実際に写っていた割合。<strong>48種中23種が0%</strong> ── 分布していても道端からは見えない種が半数近くある。</p>
...
function showTip(el) {
  ...
  tip.innerHTML =
    ...
    (TOUCH
      ? '<a class="hint gmap" href="' + panoUrl(el.dataset.lat, el.dataset.lon) +
        '" target="_blank" rel="noopener">この座標の地図を開く（ペグマンでSV）</a>'
      : '<div class="hint">クリックでこの座標の地図へ（ペグマンでSV）</div>');
  // タッチでは CSS 側で画面下部に固定する。ここで座標を書くと打ち消してしまう。
  if (!TOUCH) {
...
/* スマホにはホバーが無い。写真をツールチップだけに置くと、
   タップ＝いきなり別タブでGoogleマップ、になって写真に辿り着けない
   （実際にそうなっていた）。
   タッチ環境では 1回目のタップで写真を出し、地図へは
   ツールチップ内のリンクから明示的に開いてもらう。 */
ptsG.addEventListener('click', e => {
  if (!e.target.classList.contains('pt')) return;
  if (!TOUCH) { openPano(e.target); return; }
  select(e.target.dataset.sp);
  showTip(e.target);
});
// 地図の余白をタップしたらツールチップを閉じる
if (TOUCH) mapEl.addEventListener('click', e => {
  if (!e.target.classList.contains('pt')) hideTip();
});
