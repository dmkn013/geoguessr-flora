...
**点は「その座標にその植物がある」という確認済みの記録ではない。**
`ranges.py` に手で定義した分布域（矩形）の中から、
`sample_points.py` が乱数で散らして陸地判定を通しただけのもの。

- **本当なこと**: 矩形＝分布域は実際の知識。
  ユーカリの東部豪州、アブラヤシのスマトラ〜ボルネオ、
  ポルトガルのユーカリ植林（罠）などは意図して置いている。
- **本当でないこと**: 個々の点。スマトラの矩形内の1点は、
  市街地や川や水田に落ちているかもしれない。

つまり点は**分布の濃さを見るための表示**であって、2,785件の観測記録ではない。
教材としての主張は「この植物ならこの地域」という**面のレベル**で成り立っている。
点をクリックするとその座標の地図が開くので、
**実際に何が生えているかはペグマンを落として自分で確かめる**という使い方になる。
