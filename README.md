...
GeoGuessr の植生メタを、**実測**で確かめた資料。

このリポジトリには**性格の違う2つの調査**が入っている。

| | 48種アトラス | ランダム10,001枚 |
|---|---|---|
| 問い | この種はどこに分布するか | 道端に実際は何が映るか |
| 集め方 | 種を決めて分布域から引く | 世界の陸地から一様に引く |
| 規模 | 48種 / 2,785点 | 10,001枚 / 53タグ |
| 判定 | 92点を目視で実在確認 | **全10,001枚を目視判定** |

後者が今回の主眼で、前者は「分布域を先に仮定している」という限界がある。
両方を残しているのは、**同じ植物でも答えが食い違う**ため（後述）。

## 分かったこと

- **道端の70%には識別できる植物が映っていない**。道路と空、市街地、逆光、
  あるいは樹木は写っていても属が特定できない。植生メタが使える場面は
  思っているより少ない。
- **よく見る植物ほど場所を絞れない**。マツ属は最多の685枚だが46地域に散らばり、
  最頻地域を当てても19%。逆にヨモギ属は16枚しかないが75%が米国北西部。
- **地域差が20倍**。豪州南西部は84.8%の写真で何か読めるが、米国中西部は4.0%。
  アラビア半島（7.1%）と米国中西部では植生を見る戦略自体を捨てたほうが早い。
- **2つ以上の植物が同時に映るのは4.7%**。複数の手がかりを突き合わせる場面は
  実戦ではほぼ来ない。

詳細と図は `dist/analysis.html`。

### 2つの調査が食い違った例

分布域を狙って引いた48種調査では、9種が「40回引いて0回」だった。
ところがランダム調査では、それらが**別の場所で**見つかっている
（リュウゼツラン→アルジェリア、オリーブ→チュニジア、カラマツ→アルプス、
イトスギ→トルコ、竹→タイ、ユッカ→南カリフォルニア、ナツメヤシ→インド、
カエデ→ノルウェー、ゴムノキ→カンボジア）。

**分布域の中心は、しばしば幹線道路から外れている**（プランテーション、山地、保護区）。
「分布している」と「道から見える」は別の問題だと分かる。

## 成果物

`dist/` をブラウザで開く。GitHub Pages でも配信できる。

| ファイル | 中身 |
|---|---|
| `dist/index.html` | 入口 |
| `dist/analysis.html` | 10,001枚の分析レポート（図4点） |
| `dist/tag_atlas.html` | 44タグの分布地図。**点をタップするとその地点の実写が出る** |
| `dist/flora_atlas.html` | 48種の分布と見分け方・罠（6MB） |

タグ地図の写真は `dist/thumbs/`（17MB / タグごとに53分割）に置く。
リポジトリには含めていないので、`python src/build_thumbs.py` で作る
（元画像が手元にある場合のみ）。
...
| ファイル | 役割 |
|---|---|
| `src/collect_random.py` | 世界の陸地から面積一様に引いて車載写真を集める（4並列 / 1地点5枚） |
| `src/tag_sheet.py` | 4枚組のシートを出してタグを記録する。並列ワーカー対応 |
| `src/normalize_tags.py` | 表記ゆれ（マツ科/マツ属など）を統合する |
| `src/build_tagmap.py` | `tag_atlas.html` を組み立てる |
| `src/build_thumbs.py` | 点をタップしたとき出すサムネイルをタグごとに書き出す |
| `src/regions.py` | 座標を61地域に分類する（拡散度の計算に使う） |
...
**結果: 1,918枚を判定し、92点が確認済みとして残った（25種）。**
地図上では白い縁の点として表示され、ホバーでその地点の実写が出る。
...
| `src/encounter.py` | **遭遇率**を出す（判定の採用/却下から算出） |
| `src/findings.py` | 収集中に気づいた「狙っていない種・特徴」を溜める |
| `src/regions.py` | 分布域の矩形に地理的な名前を付ける |
...
### 遭遇率 ── 「分布している」と「実際に見える」は別

判定の内訳から **その地域の道端で実際に画面に写る割合** を出す。

    採用 / (採用 + 却下)   ＝ 写真があった地点のうち、実際に写っていた率

分布の有無（0/1）だけだと、**知識としては正しいが実戦で当たらないメタ**を
見分けられない。例えばユーカリは豪州東岸なら高頻度で見えるが、
ポルトガルの植林は「あるにはある」程度で、同じ重みでは覚えられない。

標本が少ないうちは大きく振れるので、点推定だけでなく
**Wilson score 区間**と件数を併記する。
`0/3` は「0%」ではなく「まだ分からない（上限56%）」であり、
`0/22` なら「多くても15%程度」と言える。この違いが出るようにしている。

### 収集中の気づき

数百枚の車載写真を見ていると、**狙っていない種**が繰り返し目に入る。
「この地域ではこれがよく写る」はそれ自体がメタになりうるので、
`findings.py` に溜めて後から採否を決める
（検証せずに `species.py` へ足すと教材の中身が担保されないため）。

判定画面の各カードにもメモ欄があり、判定の流れを止めずに書ける。
`--apply` で findings に取り込まれる（出典の画像IDつき）。
...
- 遭遇率の実測を踏まえた `species.py` の罠テキスト見直し。
  ユーカリは実測に合わせて書き直した（圏外の出現率は1.7%で、
  「世界中に植林されているのが最大の罠」は過大だった）が、他の種は未点検。
- コルクガシの参考写真が、剥いだ後の赤い幹を捉えていない。
